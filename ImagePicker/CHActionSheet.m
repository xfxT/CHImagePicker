...
#define kTag 111112
@interface CHActionSheet () <UITableViewDelegate, UITableViewDataSource>
...
@implementation CHActionSheet {
    CHActionSheetItemClickHandle _clickHandle;
    CHActionSheetItemClickHandle _dismissHandle;
}
...
+ (instancetype)actionSheetWithCancelTitle:(NSString *)cancelTitle alertTitle:(NSString *)alertTitle SubTitles:(NSString *)title,...NS_REQUIRES_NIL_TERMINATION {
    CHActionSheet *sheet = [[CHActionSheet alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (section == 0) {
        ...
        NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:self.alertTitle];
        ...
        [string addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:[UIScreen mainScreen].bounds.size.width > 320 ? 14 : 13] range:NSMakeRange(0, self.alertTitle.length)];
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(30, 3, [UIScreen mainScreen].bounds.size.width - 30 * 2, 52)];
        label.attributedText = string;
        label.numberOfLines = 0;
        [view addSubview:label];
        
        CALayer *layer = [CALayer layer];
        layer.frame = CGRectMake(0, 64 - 0.5, [UIScreen mainScreen].bounds.size.width, 0.5);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.font = [UIFont systemFontOfSize:[UIScreen mainScreen].bounds.size.width > 320 ? 16 : 15];
    if(indexPath.section == 0) {
        NSString *subTitle = self.subTitlesArray[indexPath.row];
        if ([subTitle containsString:@"删除"] || [subTitle containsString:@"delete"]) {
            cell.textLabel.textColor = [[UIColor redColor] colorWithAlphaComponent:0.6];
        } else {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return [UIScreen mainScreen].bounds.size.width > 320 ? 55 : 50.f;
}
...
- (void)show {
    ...
    self.bgView.alpha = 0.0;
    self.tableView.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height, [UIScreen mainScreen].bounds.size.width, [self heightForTableView]);
...
- (CGFloat)heightForTableView {
    CGFloat rowHeight = [UIScreen mainScreen].bounds.size.width > 320 ? 55 : 50;
    return  rowHeight * (self.subTitlesArray.count + 1) + 5.0f + ([self.alertTitle length] != 0 ? 60 : 0);
}
